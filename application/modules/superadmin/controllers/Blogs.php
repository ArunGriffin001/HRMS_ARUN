...
/**********************   ADD BLOG TAG SECTION ************************/
    ...
    */
    ...
    /**********************   ADD BLOG RSS SECTION ************************/
    /**
    * Sort Description.
    * function name:  addRSS  
    * Details: sload add form
    * @return: 
    * 
    */
    public function addRSS(){
        $data['header_title']="HRMS - Soylent Corp";
        $data['page_title_top']="RSS";
        $data['page_heading']="Add RSS";
        $this->superadmin_template->load('superadmin_template','superadmin/blog/rss_list/add', $data);
    }
    ...
    * 
    */
    public function submmitRSS(){
        
        $this->form_validation->set_rules('rss_title', 'RSS name','trim|required'
        );
        ...
        if ($this->form_validation->run() == FALSE) 
        {
            ...
            $this->session->set_flashdata('error_message', $errors);
            $this->session->set_flashdata('errorclass', 'danger');
            redirect(base_url('superadmin/blogs/blog-rss/add'));
        }
        else
        {
            extract($this->input->post());
            //echo $slug; die;
            $addedData = array(
                    'rss_title'         => trim($rss_title),
                    'created_at'    => date('Y-m-d h:i:s'),
                    'updated_at'    => date('Y-m-d h:i:s'),
            );
            $tag_id = $this->Mod_Common->insertData('hs_blog_rss', $addedData);
            if($tag_id){
                $errors = 'Record added successfully';
                $this->session->set_flashdata('error_message', $errors);
                $this->session->set_flashdata('errorclass', 'success');
                redirect(base_url('superadmin/blogs/blog-rss/list'));
            }else{
                $errors = 'Record not insert something is wrong';
                $this->session->set_flashdata('error_message', $errors);
                $this->session->set_flashdata('errorclass', 'danger');
    ...
    */
    public function rssList(){
        $data['header_title']="HRMS - Soylent Corp";
        $data['page_title_top']="RSS";
        $data['page_sub_heading']="RSS List";
        $this->superadmin_template->load('superadmin_template','superadmin/blog/rss_list/list', $data);
    }
    /**
    * Sort Description.
    * function name:  rssAjaxlist  
    * Details: show record in datatable
    * @return: array
    * 
    */
    public function rssAjaxlist(){
        $start   =  $this->input->get('start'); // get promo code Id
        $length  =  $this->input->get('length'); // get promo code Id
        $draw    =  $this->input->get('draw'); // get promo code Id
        $order   =  $this->input->get('order');
        ...
        $totalRecord      = $this->RSS_model->rssAjaxlist(true);
        $getRecordListing = $this->RSS_model->rssAjaxlist(false,$start,$length, $column_name, $order[0]['dir']);
        $recordListing = array();
        $content='[';
        $i=0;       
        $srNumber=$start; 
        ...
        if(!empty($getRecordListing)) {
            ...
            foreach($getRecordListing as $recordData) {
                    ...
                    $content .='[';                    
                    $recordListing[$i][0]= $srNumber+1;
                    $recordListing[$i][1]= $recordData->rss_title;
                    $recordListing[$i][2] = dateTime($recordData->created_at);
                    $table = 'hs_blog_rss';
                    $field = 'status';
                    $field_name = 'rss_id';
                    ...
                    if($recordData->status == "Deactive"){
                        $status = 'Active';
                        $actionStatus ='<a class="btn btn-danger waves-effect" style="width: 75%;" href="javascript:void(0);" onclick="changestatus('.$recordData->rss_id.', \''.$status.'\', \''.$urls.'\' , \''.$table.'\', \''.$field.'\', \''.$field_name.'\');" title="Deactive">Deactive</a>';

                    }else { 
                        $status = 'Deactive';
                        $actionStatus ='<a class="btn btn-primary waves-effect" style="width: 75%;" href="javascript:void(0);" onclick="changestatus('.$recordData->rss_id.', \''.$status.'\', \''.$urls.'\' , \''.$table.'\', \''.$field.'\', \''.$field_name.'\');" title="Active">Active</a>';
                    }
                    $recordListing[$i][3]= $actionStatus;
                    $actionLink = '';
                    
                    $edit_slider_url = base_url(ADMIN_URL).'blogs/blog-rss/edit/'.encode($recordData->rss_id);
                    $actionLink .= '<a href="'.$edit_slider_url.'" title="Edit" class="btn btn-icon waves-effect waves-light fa-new-grey m-b-5"><i class="fa fa-edit"></i></a>';
    ...
    */
    public function editRss($id) {
        $data['header_title']="HRMS - Soylent Corp";
        $data['page_title_top']="RSS";
        $data['page_heading']="Edit RSS";
        $record = rowData(TABLE_BLOG_RSS, array('rss_id'=>decode($id)));
        $data['record'] = $record;
        $this->superadmin_template->load('superadmin_template','superadmin/blog/rss_list/edit', $data);
    }
    ...
    */
    public function updateRSS($id){
        ...
        if ($this->form_validation->run() == FALSE) 
        {
            ...
            $this->session->set_flashdata('error_message', $errors);
            $this->session->set_flashdata('errorclass', 'danger');
            redirect(base_url(ADMIN_URL).'blogs/blog-rss/edit/'.$id);
        }
        else
        {
            extract($this->input->post());
            ...
            $tag_id = $this->Mod_Common->updateData(TABLE_BLOG_RSS,array('rss_id'=>decode($id)), $updatedData);
            if($tag_id){
                $errors = 'RSS name updated successfully';
                $this->session->set_flashdata('error_message', $errors);
                $this->session->set_flashdata('errorclass', 'success');
                redirect(base_url(ADMIN_URL).'blogs/blog-rss/list/');
            }else{
                $errors = 'Record not update something is wrong';
                $this->session->set_flashdata('error_message', $errors);
                $this->session->set_flashdata('errorclass', 'danger');
    ...
    */
    public function addBlog(){
        ...
        $blog_rss_arr = $this->Mod_Common->selectData(TABLE_BLOG_RSS,array('status'=>'Active'));
        $data['blog_rss_arr'] = $blog_rss_arr;
    ...
    public function submitBlog(){
        ...
        else
        {
            ...
            if($blog_id){
                ...
                /* Blog tag option*/
                foreach ($rss_id as $key => $rss_val) 
                {
                    $batch_rss_option_array[] = array('blog_id'=>$blog_id,'blog_rss_id'=>$rss_val,'created_at'=>$add_date,'updated_at'=>$add_date);
                }

                if(!empty($batch_rss_option_array))
                {
                    $this->Mod_Common->insertbatchdata(TABLE_BLOG_RSS_OPTION ,$batch_rss_option_array);
                }
    ...
    public function blogAjaxList(){
        ...
        if(!empty($order)){ 
            if($order[0]['column']==1){
                ...
                $column_name='hs_blog_rss.rss_title';
        ...
        if(!empty($getRecordListing)) {
            ...
            foreach($getRecordListing as $recordData) {
                    ...
                    $rss_id = implode(",", (json_decode($recordData->rss_id)));
                    $query3 = "SELECT * FROM hs_blog_rss WHERE `rss_id` IN($rss_id)";
                    $tag_id_arr = $this->Mod_Common->customQuery($query3);
                    $rss_name = array_column($tag_id_arr, 'rss_title');
                    $recordListing[$i][5]= implode(',', $rss_name);
    ...
    */
    public function editBlog($id) {
        ...
        $blog_rss_option_arr = $this->Mod_Common->selectData(TABLE_BLOG_RSS_OPTION, array('blog_id'=>decode($id)));
        ...
        $data['blog_rss_option'] = $blog_rss_option_arr;
    ...
    */
    public function updateBlog($id){
        ...
        $this->form_validation->set_rules('tag_id[]', 'Tag','trim|required'
         ...
         $this->form_validation->set_rules('rss_id[]', 'RSS','trim|required'
        );
        ...
        else
        {
            ...
            if($update_id){
                ...
                $tag_opt_id = $this->Mod_Common->deleteData(TABLE_BLOG_TAG_OPTION, array('blog_id' => decode($id)));

                $rss_opt_id = $this->Mod_Common->deleteData(TABLE_BLOG_RSS_OPTION, array('blog_id' => decode($id)));
                ...
                foreach ($rss_id as $key => $rss_val) 
                {
                    $batch_rss_option_array[] = array('blog_id'=>decode($id),'blog_rss_id'=>$rss_val,'created_at'=>$add_date,'updated_at'=>$add_date);
                }
