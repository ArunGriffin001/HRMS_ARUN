...
class LeaveType_model extends CI_Model {
    ...
    public function leaveTypeAjaxList($isCount=false,$start=0,$stop=0, $column_name='',$order=''){
        
        if(!empty($column_name) && $column_name=='leave_type_name' ){
            $orderby_name = 'leave_type_name';
        }else if(!empty($column_name) && $column_name=='total_leave' ){
            $orderby_name = 'total_leave';
        }else if(!empty($column_name) && $column_name=='status' ){
            $orderby_name = 'status';
        }else if(!empty($column_name) && $column_name=='created_at' ){
            $orderby_name = 'created_at';
        }else{
            $orderby_name = 'leave_cat_id';
        }
        $search = $this->input->get('search');
        /*$this->db->query("SET sql_mode=(SELECT REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', ''));");*/
        $this->db->select('*');
        $this->db->from(TABLE_LEAVE_TYPE);
        ...
        if(!empty($search['value'])){
            ...
            $this->db->where('(`leave_type_name` LIKE "%'.$this->db->escape_like_str($search_info).'%" OR `total_leave` LIKE "%'.$this->db->escape_like_str($search_info).'%" OR `status` LIKE "%'.$this->db->escape_like_str($search_info).'%" OR `created_at` LIKE "%'.$this->db->escape_like_str($search_info).'%")',NUll);
        }
