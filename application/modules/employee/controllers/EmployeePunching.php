...
class EmployeePunching extends MX_Controller  {
    ...
    * 
    */
    public function breakInOut(){

        
       $returnData = false;
        //echo json_encode($returnData);
       //$returnData = array('isSuccess' => true, 'status'=>'2');
        $break_status = $this->input->post('break_status');
        ...
        if(!empty($today_puch)){ // today record exist the update date
            ...
            if($break_status == '1'){
                ...
                $status = $this->Mod_Common->insertData('hs_employee_punching_log', $addedData2);

                $returnData = array('isSuccess' => true, 'status'=>$break_status);
